...
public class EJ3 {
    ...
    public static void main(String[] args) {
        ...
        boolean[] mayor = new boolean[3];
        int[] imc = new int[3];
        PersonaServicio s1 = new PersonaServicio();
        for (int i = 0; i < 3; i++) {
            p1[i] = s1.crearPersonas();
            mayor[i] = s1.esMayorDeEdad(p1[i].getEdad());
            imc[i] = s1.calcularIMC(p1[i].getPeso(), p1[i].getAltura());
            System.out.println(p1[i].getEdad());
            System.out.println(p1[i].getPeso());
        }
        for (int i = 0; i < 3; i++) {
            if (mayor[i] == true) {
                System.out.println(p1[i].getNombre() + " es mayor de edad");

            } else {
                System.out.println(p1[i].getNombre() + " es menor de edad");
            }
            if (imc[i] == -1) {
                System.out.println(p1[i].getNombre() + " esta por debajo de su peso ideal");

            }
            if (imc[i] == 0) {
                System.out.println(p1[i].getNombre() + " esta en su peso ideal");
            }
            if (imc[i] == 1) {
                System.out.println(p1[i].getNombre() + " esta por encima de su peso ideal");
            }
            System.out.println("---------------  ");
        }
    }
